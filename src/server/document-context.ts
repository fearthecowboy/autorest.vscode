...
export class DocumentContext extends EventEmitter implements IFileSystem {
  ...
  private _codeGenOutputs = new Map<string, { type: string, content: string }>();

  public getGeneratedCode(): Map<string, { type: string, content: string }> { return this._codeGenOutputs; }
  ...
  private async createAutoRestCodeGenInstance(): Promise<AutoRest> {
    const result = await create(this, this.configurationFile);
    result.AddConfiguration({ "csharp": true });
    ...
    const opDirName: string = (this.configurationFile) ? path.basename(path.dirname(this.configurationFile)) : 'out';
    ...
    result.AddConfiguration({ "output-folder": opPath });

    this.Manager.listenForResults(result);
    result.GeneratedFile.Subscribe((_instance, artifact) => {
      this._codeGenOutputs.set(artifact.uri, artifact);
    });
    return result;
  }
  ...
  constructor(public Manager: OpenApiDocumentManager, public RootUri: string, public configurationFile?: string) {
    ...
    this.codeGen = this.createAutoRestCodeGenInstance();
  }
