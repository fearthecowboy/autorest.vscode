...
import { JsonPath, SourceMap } from '../lib/ref/source-map';
...
export class DocumentAnalysis {
  public static async Create(host: AutoRestManager, documentUri: string): Promise<DocumentAnalysis | null> {
  ...
  public GetJsonPathFromJsonReferenceAt(position: Position): string | null {
    ...
    if (!fullyResolvedAndMergedDefinitionLocation) { console.log("reverse lookup failed (no big deal, may just hover empty space)"); return null; }

    const path = fullyResolvedAndMergedDefinitionLocation.path;
    if (!path) { console.warn("no object path for that location"); return null; }
